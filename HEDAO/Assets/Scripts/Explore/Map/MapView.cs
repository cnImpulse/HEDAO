    ...
    public class MapView : MonoBehaviour
    {
        ...
        public void SetAttainableNodes()
        {
            ...
            else
            {
                ...
                foreach (Vector2Int point in Map.path)
                {
                ...
                Vector2Int currentPoint = Map.path[Map.path.Count - 1];
        ...
        public virtual void SetLineColors()
        {
            ...
            if (Map.path.Count <= 1) return;

            for (int i = 0; i < Map.path.Count - 1; i++)
            {
                Vector2Int current = Map.path[i];
                Vector2Int next = Map.path[i + 1];
        ...
        {
            MapConfig config = GetConfig(Map.configName);
        ...
        private void SendPlayerToNode(MapNode mapNode)
        {
            ...
            Map.path.Add(mapNode.Node.point);
            SetAttainableNodes();
            SetLineColors();
